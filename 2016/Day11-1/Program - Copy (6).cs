...
// (77) 1>2:CM.RM, 2>1:CM, 1>2:CM.TM, 2>3:CM.OM, 3>2:CM, 2>3:CM.PM, 3>2:CM, 2>1:CM, 1>2:RG.TG, 2>3:RM.TM, 3>2:RM, 2>1:RM.RG, 1>2:CG.CM, 2>3:CM, 3>2:TM, 2>1:TM.TG, 1>2:RG.RM, 2>3:RM, 3>4:CM.OM, 4>3:CM, 3>2:CM, 2>1:CM.CG, 1>2:OG.PG, 2>3:PG.RG, 3>4:PM.RM, 4>3:PM, 3>2:PM.PG, 2>3:OG.PG, 3>4:OG.RG, 4>3:OM.OG, 3>2:OM, 2>3:OM.PM, 3>4:OG.OM, 4>3:RM.RG, 3>4:PG.PM, 4>3:OM.OG, 3>4:OG.RG, 4>3:PM, 3>4:OM.PM, 4>3:PM, 3>4:PM.RM, 4>3:OM.OG, 3>2:OM.OG, 2>1:OM.OG, 1>2:CM.OM, 2>1:CM, 1>2:CM.TM, 2>1:CM, 1>2:OG.TG, 2>3:OM.TM, 3>2:OM, 2>1:OM.OG, 1>2:CG.OG, 2>3:CG.TG, 3>4:CG.TG, 4>3:PM, 3>4:PM.TM, 4>3:PM.PG, 3>2:PM.PG, 2>1:PM, 1>2:OM.PM, 2>3:OG.PG, 3>4:OG.PG, 4>3:RM, 3>2:RM, 2>1:OM, 1>2:CM.OM, 2>3:CM.OM, 3>2:CM, 2>3:CM.RM, 3>4:CM.OM, 4>3:CM, 3>4:CM.RM, 4>3:CM, 3>2:CM, 2>3:CM.PM, 3>4:CM.PM
...
void CheckActions()
{
    ...
    if (layout.Any(l => l.Value < floor))
    {
        for (int i = 0; i < localMicrochips.Length; i++)
        {
            for (int j = 0; j < localDevices.Length; j++)
            {
                if (localMicrochips[i].Key != localDevices[j].Key)
                    newActions.Enqueue((floor, floor - 1, new[] { localMicrochips[i].Key, localDevices[j].Key }));
            }
        }
    }

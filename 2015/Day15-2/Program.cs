...
{
    var calories = int.Max(0, item.Sum(i => i.Amt * ingredients[i.Name].Calories));
    ...
    return (Desc: string.Join(", ", item), Total: total, Calories: calories);
});

Console.WriteLine(values.Where(v => v.Calories == 500).MaxBy(v => v.Total));

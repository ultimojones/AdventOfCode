...
for (int i = 0; ; i++)
{
    List<((int X, int Y) Cur, (int X, int Y) Next)> next = grid.AsParallel().Select(elf =>
    {
    ...
    int moved = 0;
    grid = next.GroupBy(k => k.Next).SelectMany(g =>
    {
        var first = g.First();
        if (g.Count() == 1)
        {
            if (first.Cur != first.Next)
                moved++;
            return new[] { first.Next };
        }
        else
        {
            moved += g.Count();
            return g.Select(p => p.Cur);
        }
    }).ToList();
